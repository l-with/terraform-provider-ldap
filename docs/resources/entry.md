...
Pre-stage a computer account in Active Directory. The `recursive_delete` option is required
because AD creates child objects when a machine joins the domain.

```terraform
resource "ldap_entry" "computer_account" {
  dn = "CN=WEBSERVER01,OU=Servers,DC=corp,DC=example,DC=com"
  data_json = jsonencode({
    objectClass        = ["computer"]
    sAMAccountName     = ["WEBSERVER01$"]
    userAccountControl = ["4096"] # WORKSTATION_TRUST_ACCOUNT
  })

  # Required for AD computer objects - they have child objects after domain join
  recursive_delete = true

  lifecycle {
    ignore_changes = [data_json] # AD modifies attributes after domain join
  }
}
```
...
- `recursive_delete` (Boolean) Enable recursive deletion using LDAP Tree Delete Control (OID 1.2.840.113556.1.4.805). Required for deleting Active Directory objects that have child objects (e.g., computer accounts after domain join). Defaults to `false`.
